...
/***  File:  udpopt.c
/***
/***  Function: present UDP control window
/***
/***  Revisions:
/***
/******************************************************************************/
...
udprefresh (int i)
{
    ...
    case UDP_STATUS:
        ...
        break;
    case UDP_DROPPED:
        sprintf (buf, "> UDP trans dropped: %d (%d%% | %d%%)", udpDropped, udpDropped * 100 / udpTotal, /* (udpTotal always > 0) */
                 udpRecentDropped * 100 / UDP_RECENT_INTR);
        break;
    case UDP_SEQUENCE:
        sprintf (buf, "Sequence checking is %s", (udpSequenceCheck) ?
                 "ON" : "OFF");
        break;
    case UDP_DEBUG:
...
udpaction (W_Event * data)
{
    ...
    case UDP_DROPPED:
        ...
        break;
    case UDP_SEQUENCE:
        udpSequenceCheck = !udpSequenceCheck;
...
#ifdef DOUBLE_UDP
        if (udpClientReceive > MODE_DOUBLE)
            udpClientReceive = 0;
#else
        if (udpClientReceive >= MODE_DOUBLE)
            udpClientReceive = 0;
    ...
    case UDP_FORCE_RESET:
        ...
        commSwitchTimeout = 0;
        udpClientSend = udpClientReceive = udpSequenceCheck = udpTotal = 1;
