...
/***  File:  short.c
/***
/***  Function: This file implements all SHORT_PACKETS functions
/***
/***  Revisions: Heiko Wengler 19.07.93
/***
/******************************************************************************/
...
handleSelfShip (struct youss_spacket *packet)
{
    ...
    else
    {
        ...
        if (F_self_19flags)
        {
            me->p_flags &= ~(PFGREEN|PFYELLOW|PFRED|PFBEAMUP|PFBEAMDOWN|PFPLOCK|
                            PFPLLOCK|PFPRESS|PFTRACT|PFORBIT|PFDOCK);

            f = (unsigned char) packet->pad1;

            /* I don't like what I see, but I never found better look */
            if (f >= 162)     { me->p_flags |= PFORBIT; f -= 162; }
            else if (f >= 81) { me->p_flags |= PFDOCK; f -= 81; }

            if (f >= 54)      { me->p_flags |= PFTRACT; f -= 54; }
            else if (f >= 27) { me->p_flags |= PFPRESS; f -= 27; }

            if (f >= 18)     { me->p_flags |= PFPLOCK; f -= 18; }
            else if (f >= 9) { me->p_flags |= PFPLLOCK; f -= 9; }

            if (f >= 6)      { me->p_flags |= PFBEAMUP; f -= 6; }
            else if (f >= 3) { me->p_flags |= PFBEAMDOWN; f -= 3; }

            if (f == 2)      { me->p_flags |= PFGREEN; }
            else if (f == 1) { me->p_flags |= PFYELLOW; }
            else if (f == 0) { me->p_flags |= PFRED; }
        }
        else if (F_self_8flags)
            me->p_flags =
                (me->p_flags & 0xffffff00) | (unsigned char) packet->pad1;
        else if (F_self_8flags2)
        {
            unsigned int new_flags =
                me->
                p_flags & ~(PFSHIELD | PFREPAIR | PFCLOAK | PFGREEN | PFYELLOW
                            | PFRED | PFTRACT | PFPRESS);
...
handleVPlayer (unsigned char *sbuf)
{
    ...
    {                           /* Short Header + Extended */
        ...
        for (i = 0; i < numofplayers; i++)
        {
             ...
             * [007] */
            if (F_dead_warp && pl->p_speed == 14 && x == 501 && y == 501
...
handleSWarning (struct warning_s_spacket *packet)
{
    ...
    case BOMB_INEFFECTIVE:
        sprintf (buf, "Weapons Officer: Bombing is ineffective.  Only %d armies are defending.", (int) packet->argument);       /* nifty info feature
                                                                                                                                 * 2/14/92 TMC */
        warning (buf);
        break;
    case BOMB_TEXT:
        sprintf (buf,
                 "Weapons Officer: Bombarding %s...  Sensors read %d armies left.",
    ...
    case BEAMUP_TEXT:
        ...
        break;
    case BEAMUP2_TEXT:
        sprintf (buf, "Beaming up. (%d/%d)", (unsigned char) packet->argument,
                 (unsigned char) packet->argument2);
        warning (buf);
        break;
    ...
    case SBREPORT:
        sprintf (buf,
                 "Transporter Room:  Starbase %s reports all troop bunkers are full!",
                 players[packet->argument].p_name);
        warning (buf);
        break;
    case ONEARG_TEXT:
        ...
        {
            sprintf (buf, vari_texts[packet->argument],
                     (unsigned char) packet->argument2);
            warning (buf);
        }
        break;
    ...
    case BEAM_U_TEXT:
        sprintf (buf,
                 "Transfering ground units.  (%d/%d) Starbase %s has %d armies left",
    ...
    case SBRANK_TEXT:
        sprintf (buf,
                 "You need a rank of %s or higher to command a starbase!",
                 ranks[packet->argument].name);
        warning (buf);
        break;
    ...
    case SBDOCKDENIED_TEXT:
        sprintf (buf,
                 "Starbase %s: Permission to dock denied, all ports currently occupied.",
                 players[packet->argument].p_name);
        warning (buf);
        break;
    case SBLOCKSTRANGER:
    ...
    case SBLOCKMYTEAM:
        sprintf (buf, "Locking onto %s (%c%c) (docking is %s)",
                 ...
                 teamlet[players[packet->argument].p_team],
                 shipnos[players[packet->argument].p_no],
                 (players[packet->argument].
                  p_flags & PFDOCKOK) ? "enabled" : "disabled");
        warning (buf);
        break;
    case DMKILL:
...

        {
            ...
            if (armies == 0)
            {
                (void) sprintf (msg.mesg,
                                "GOD->ALL %s (%c%c) was kill %s for %s (%c%c)",
                                players[victim].p_name,
                                teamlet[players[victim].p_team],
                                shipnos[victim], killmess,
            ...
            {
                (void) sprintf (msg.mesg,
                                "GOD->ALL %s (%c%c+%d armies) was kill %s for %s (%c%c)",
                                players[victim].p_name,
                                teamlet[players[victim].p_team],
                                shipnos[victim], armies, killmess,
                                players[killer].p_name,
                                teamlet[players[killer].p_team],
                                shipnos[killer]);
    ...
    case DMBOMB:
        {
            ...
            (void) sprintf (buf, "%-3s->%-3s",
                            ...
                            teamshort[planets
                                      ...
                                      pl_owner]);
            (void) sprintf (buf1,
                            "We are being attacked by %s %c%c who is %d%% damaged.",
                            players[packet->argument].p_name,
                            teamlet[players[packet->argument].p_team],
                            shipnos[packet->argument], arg3);
    ...
    case DMTAKE:
        {
            ...
            (void) sprintf (buf1, "%-3s->%-3s",
                            planets[(unsigned char) packet->argument].pl_name,
                            teamshort[players[packet->argument2].p_team]);
            (void) sprintf (msg.mesg, "%s %s", buf1, buf);
#endif
            ...
            msg.m_recpt = (unsigned char) (players[packet->argument2].p_team);
            msg.m_from = 255;
            handleMessage (&msg);
        }
        break;
...
#else
            (void) sprintf (msg.mesg,
                            "GOD->ALL %s (%c%c) was kill %0.2f for the GhostBusters",
                            players[(unsigned char) packet->argument].p_name,
                            teamlet[players[(unsigned char) packet->argument].
                                    p_team],
    ...
    case INLDMKILLP:
        {
            ...
            (void) sprintf (msg.mesg,
                            "GOD->ALL %s(%s) (%c%c%s) killed by %s (%c)",
                            players[(unsigned char) packet->argument].p_name,
                            shiptype[players
                                     [(unsigned char) packet->argument].
                                     p_ship.s_type],
                            teamlet[players[(unsigned char) packet->argument].
                                    p_team],
                            shipnos[(unsigned char) packet->argument], buf,
                            planets[(unsigned char) packet->argument2].
                            pl_name,
                            teamlet[planets
                                    [(unsigned char) packet->argument2].
    ...
    case INLDMKILL:
        {
            ...
            {
                (void) sprintf (msg.mesg,
                                "GOD->ALL %s(%s) (%c%c) was kill %0.2f for %s(%s) (%c%c)",
                                players[victim].p_name,
                                shiptype[players[victim].p_ship.s_type],
                                teamlet[players[victim].p_team],
                                shipnos[victim], kills,
                                players[killer].p_name,
            ...
            {
                (void) sprintf (msg.mesg,
                                "GOD->ALL %s(%s) (%c%c+%d armies) was kill %0.2f for %s(%s) (%c%c)",
                                players[victim].p_name,
                                shiptype[players[victim].p_ship.s_type],
                                teamlet[players[victim].p_team],
                                shipnos[victim], armies, kills,
                                players[killer].p_name,
                                shiptype[players[killer].p_ship.s_type],
                                teamlet[players[killer].p_team],
                                shipnos[killer]);
    ...
    case STEXTE_STRING:
        {
            ...
            warning (warn->mesg);
            s_texte[(unsigned char) warn->pad2] =
                (char *) malloc (warn->pad3 - 4);
