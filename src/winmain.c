...
/***
/***  Revisions: Copyright (C) 1993-1995 Jonathan Shekter
/***
/******************************************************************************/
...
getpwuid ()
{
    ...
    if ((WNetGetUser (NULL, name, &i) != NO_ERROR) ||
        ...
        !stricmp (name, "admin") || !stricmp (name, "root"))
    {
        /* Pretty lame ids - use the one given in the rc file */
        p = NULL;
        p = stringDefault ("login");
        if (!p || !name[0])
            return NULL;
        pw.pw_name = p;
        return &pw;
         ...
         ...)
{
    ...
    if (handle == stdout || handle == stderr)
    {
        if (fWin32s)
        {
            if (!PrintfSafe)
            {
                MessageBox (NULL, buf, "This is an fprintf to stderr/stdout",
                            MB_OK);
                return 0;
            }
            PrintfSafe = 0;
            ...
            WriteFile (GetStdHandle (STD_OUTPUT_HANDLE), buf, len, &junk,
                       NULL);
    }
    else
        ...
        FILE * handle)
{
    ...
    if (handle == stdout || handle == stderr)
    {
        if (fWin32s)
        {
            if (!PrintfSafe)
            {
                MessageBox (NULL, buf, "This is an fwrite to stderr/stdout",
                            MB_OK);
                return 0;
            }
            PrintfSafe = 0;
            if (!W_IsMapped (console))
                W_MapWindow (console);
            W_WriteText (console, 0, 0, W_White, buf, blksz * count,
                         W_RegularFont);
            W_FlushScrollingWindow (console);
            PrintfSafe = 1;
        }
        else
            WriteFile (GetStdHandle (STD_OUTPUT_HANDLE), buf, blksz * count,
                       &junk, NULL);
    }
    else
