      ...
      int y)
{
    ...
    else if (value < 10000) sprintf (buf, " %4d", value);
...
updatePStats (void)
{
    ...
    for (i = 0; i < NUM_SLIDERS; i++)
    {
        ...
        {                       /* bar decreasing */
            ...
            box (1, BX_OFF () + new_x, BY_OFF (i), old_x - new_x, SL_HEI,
                 backColor);
        ...
        {                       /* bar increasing */
            ...
            old_x = s->lastVal * SL_WID / s->diff;
            new_x = value * SL_WID / s->diff;
            box (1, BX_OFF () + old_x, BY_OFF (i), new_x - old_x, SL_HEI,
                 color);
        }
        s->lastVal = value;
    }
...
/* Taking box() from stats.c instead SRS 8/00 */
int
box (int filled,
     int x,
     int y,
